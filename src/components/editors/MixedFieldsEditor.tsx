  ...
  const renderSettings = (field: MixedField, index: number) => {
    switch (field.type) {
      ...
      case 'progress':
        return (
          <div className="space-y-2">
            <div className="flex items-center gap-2"><span className="w-16 flex-shrink-0 text-xs text-theme-muted">Current</span><LabeledNumberField value={field.current} onChange={(current) => updateField(index, { ...field, current: clampMixedFieldValue(current, field.min ?? 0, field.max) })} fieldLabel={field.currentLabel} onFieldLabelChange={(currentLabel) => updateField(index, { ...field, currentLabel })} formula={field.currentFormula} onFormulaChange={(currentFormula) => updateField(index, { ...field, currentFormula })} min={field.min ?? 0} max={field.max} compact hideStepperButtons /></div>
            <div className="flex items-center gap-2"><span className="w-16 flex-shrink-0 text-xs text-theme-muted">Minimum</span><LabeledNumberField value={field.min ?? 0} onChange={(min) => updateField(index, { ...field, min: Math.min(min, field.max), current: Math.max(Math.min(min, field.max), field.current) })} fieldLabel={field.minLabel} onFieldLabelChange={(minLabel) => updateField(index, { ...field, minLabel })} formula={field.minFormula} onFormulaChange={(minFormula) => updateField(index, { ...field, minFormula })} max={field.max} compact hideStepperButtons /></div>
            <div className="flex items-center gap-2"><span className="w-16 flex-shrink-0 text-xs text-theme-muted">Maximum</span><LabeledNumberField value={field.max} onChange={(max) => { const nextMax = Math.max(field.min ?? 0, max); updateField(index, { ...field, max: nextMax, current: Math.min(field.current, nextMax) }); }} fieldLabel={field.maxLabel} onFieldLabelChange={(maxLabel) => updateField(index, { ...field, maxLabel })} formula={field.maxFormula} onFormulaChange={(maxFormula) => updateField(index, { ...field, maxFormula })} min={field.min ?? 0} compact hideStepperButtons /></div>
